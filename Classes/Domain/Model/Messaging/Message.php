<?php
/***************************************************************
 *  Copyright notice
 *
 *  (c) 2010-2011 punkt.de GmbH - Karlsruhe, Germany - http://www.punkt.de
 *  Authors: Daniel Lienert, Michael Knoll, Christoph Ehscheidt
 *  All rights reserved
 *
 *  For further information: http://extlist.punkt.de <extlist@punkt.de>
 *
 *
 *  This script is part of the TYPO3 project. The TYPO3 project is
 *  free software; you can redistribute it and/or modify
 *  it under the terms of the GNU General Public License as published by
 *  the Free Software Foundation; either version 2 of the License, or
 *  (at your option) any later version.
 *
 *  The GNU General Public License can be found at
 *  http://www.gnu.org/copyleft/gpl.html.
 *
 *  This script is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU General Public License for more details.
 *
 *  This copyright notice MUST APPEAR in all copies of the script!
 ***************************************************************/

/**
 * Class implements a message for errors, warnings, infos etc.
 * 
 * @author Michael Knoll 
 * @package Domain
 * @subpackage Model\Messaging
 */
class Tx_PtExtlist_Domain_Model_Messaging_Message {
	
	/**
	 * Constant for message type error
	 */
	const ERROR_MESSAGE_TYPE = 1;

	
	
	/**
	 * Constant for message type warning
	 */
	const WARNING_MESSAGE_TYPE = 2;
	
	

	/**
	 * Constant for message type info
	 */
	const INFO_MESSAGE_TYPE = 3;
	
	

	/**
	 * Holds message type
	 *
	 * @var int
	 */
	protected $messageType;
	
	
	
	/**
	 * Holds message content
	 *
	 * @var string
	 */
	protected $messageContent;
	
	
	
	/**
	 * Holds message key
	 *
	 * @var string
	 */
	protected $messageKey;
	
	
	
	/**
	 * Constructor for message
	 *
	 * @param string $messageContent
	 * @param int $messageType
	 * @param string $messageKey
	 */
	public function __construct($messageContent, $messageType='', $messageKey='') {
		$this->messageContent = $messageContent;
		$this->messageType = $messageType;
		$this->messageKey = $messageKey;
	}
	
	
	
	/**
	 * Getter for message type
	 *
	 * @return int
	 */
	public function getMessageType() {
		return $this->messageType;
	}
	
	
	
	/**
	 * Getter for message content
	 *
	 * @return string
	 */
	public function getMessageContent() {
		return $this->messageContent;
	}
	
	
	
	/**
	 * Getter for message key
	 *
	 * @return string
	 */
	public function getMessageKey() {
		return $this->messageKey;
	}
	
	
	
	/**
	 * Setter for message content
	 *
	 * @param string $messageContent
	 */
	public function setMessageContent($messageContent) {
		$this->messageContent = $messageContent;
	}
	
	
	
	/**
	 * Setter for message key
	 *
	 * @param string $messageKey
	 */
	public function setMessageKey($messageKey) {
		$this->messageKey = $messageKey;
	}
	
	
	
	/**
	 * Setter for message type
	 *
	 * @param int $messageType
	 */
	public function setMessageType($messageType) {
		$this->messageType = $messageType;
	}
	
}