...
class Tx_PtExtlist_ViewHelpers_HeaderViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper {
	 ...
	 * @param $captions
	 */
	public function render(Tx_PtExtlist_Domain_Model_List_Header_ListHeader $headers, Tx_PtExtlist_Domain_Model_List_Row $captions) {
		$output = '';
		...
		foreach ($headers as $header) {
			...
			$this->templateVariableContainer->add('sorting', $this->renderSorting($header));
			
			$output .= $this->renderChildren();
			
			$this->templateVariableContainer->remove('label');
			$this->templateVariableContainer->remove('sorting');
