...
plugin.tx_ptextlist.settings {
	...
	listConfig.demolist {
		...
		columns {
			...
			31 {
				label = Phone2
				columnIdentifier = phoneColumn2
				# default columns can only operate on _one_ dataDescription
				fieldIdentifier =  capital, phone
			}
		...
		filters {
			filterbox1 {
			    ...
			    #    action = testAction
			    #}
			    excludeFilters = filterbox2.firstLetterFilter
				filterConfigs {
...
plugin.tx_ptextlist.settings.listConfig.demolist_export < plugin.tx_ptextlist.settings.listConfig.demolist
plugin.tx_ptextlist.settings.listConfig.demolist_export {

    # show all items
    pager.itemsPerPage = 0

    # redifine columns completely
    columns >
    columns {

        10 {
            columnIdentifier = nameColumn
            label = A label especially for the download List
